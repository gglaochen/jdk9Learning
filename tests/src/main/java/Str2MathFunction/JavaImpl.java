...
public class JavaImpl {
     ...
     * 运算符优先级
     */
    private final Map<Character, Integer> operaPriority = new HashMap<>();
    ...
    public static void main(String[] args) {
        System.out.println(JavaImpl.conversion("-1e1- (1+2)^3"));
    }

    public static double conversion(String expression) {
        double result;
        JavaImpl cal = new JavaImpl();
        try {
    ...
    public boolean compare(char cur, char peek) {// 如果是peek优先级高于cur，返回true，默认都是peek优先级要低
        ...
        if (operaPriority.get(peek) >= operaPriority.get(cur)) {
            result = true;
        }
        return result;
    }
    ...
    private String calculate(String firstValue, String secondValue, char currentOp) {
        ...
        switch (currentOp) {
            ...
            case '/':
                ...
                break;
            case '^':
                result = String.valueOf(ArithHelper.squ(firstValue, secondValue));
                break;
            default:
        }
        return result;
